...
export class InitMigration1747776054610 implements MigrationInterface {
    name = 'InitMigration1747776054610'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`time_trackings\` (\`id\` int NOT NULL AUTO_INCREMENT, \`user_id\` int NOT NULL, \`task_id\` int NOT NULL, \`start_time\` timestamp NULL, \`end_time\` timestamp NULL, \`duration_hours\` float NULL, \`date\` date NOT NULL, \`is_active\` tinyint NOT NULL DEFAULT 1, \`is_paused\` tinyint NOT NULL DEFAULT 0, \`pause_time\` timestamp NULL, \`paused_duration_hours\` float NOT NULL DEFAULT '0', \`last_heartbeat\` timestamp NULL, \`auto_paused\` tinyint NOT NULL DEFAULT 0, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
