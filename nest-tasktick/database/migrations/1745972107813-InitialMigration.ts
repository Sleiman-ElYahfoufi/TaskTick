...
export class InitialMigration1745972107813 implements MigrationInterface {
    name = 'InitialMigration1745972107813'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`time_trackings\` (\`id\` int NOT NULL AUTO_INCREMENT, \`user_id\` int NOT NULL, \`task_id\` int NOT NULL, \`start_time\` timestamp NOT NULL, \`end_time\` timestamp NULL, \`session_duration\` float NULL, \`date\` date NOT NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`tasks\` (\`id\` int NOT NULL AUTO_INCREMENT, \`name\` varchar(255) NOT NULL, \`description\` text NULL, \`estimated_time\` float NOT NULL, \`dueDate\` datetime NULL, \`priority\` enum ('low', 'medium', 'high') NOT NULL DEFAULT 'medium', \`status\` enum ('todo', 'in_progress', 'completed') NOT NULL DEFAULT 'todo', \`progress\` int NOT NULL DEFAULT '0', \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`project_id\` int NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`projects\` (\`id\` int NOT NULL AUTO_INCREMENT, \`name\` varchar(255) NOT NULL, \`description\` text NOT NULL, \`deadline\` datetime NULL, \`priority\` enum ('low', 'medium', 'high') NOT NULL DEFAULT 'medium', \`detail_depth\` enum ('minimal', 'normal', 'detailed') NOT NULL DEFAULT 'normal', \`status\` enum ('planning', 'in_progress', 'delayed', 'completed') NOT NULL DEFAULT 'planning', \`estimated_time\` float NOT NULL, \`accuracy_rating\` float NOT NULL DEFAULT '0', \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`user_id\` int NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`tech_stacks\` (\`id\` int NOT NULL AUTO_INCREMENT, \`name\` varchar(255) NOT NULL, \`category\` enum ('frontend', 'backend', 'database', 'devops', 'mobile') NOT NULL, UNIQUE INDEX \`IDX_ed42032091d26293136b63a01d\` (\`name\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`time_trackings\` ADD CONSTRAINT \`FK_e17330186abd1eaae651f6c1139\` FOREIGN KEY (\`task_id\`) REFERENCES \`tasks\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        ...
        await queryRunner.query(`ALTER TABLE \`user_tech_stacks\` ADD CONSTRAINT \`FK_901f0a188a48cea3dfe6b1330ee\` FOREIGN KEY (\`user_id\`) REFERENCES \`users\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`user_tech_stacks\` ADD CONSTRAINT \`FK_554f7c46630ca7b38058f8dc337\` FOREIGN KEY (\`tech_id\`) REFERENCES \`tech_stacks\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`ai_insights\` ADD CONSTRAINT \`FK_4b982463f329037b4b7d2b4cde6\` FOREIGN KEY (\`user_id\`) REFERENCES \`users\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
    }
